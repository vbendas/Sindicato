...
export default function WorkerDashboard({ cases, timelineMap, workerId }: WorkerDashboardProps) {
  ...
  const [localEvents, setLocalEvents] = useState<Record<string, TimelineEvent[]>>({});
  ...
  const inputClass =
    "w-full bg-white border border-sindicato-charcoal/20 rounded-none text-sindicato-charcoal p-2.5 focus:border-sindicato-bordeaux focus:outline-none focus:ring-0 transition-colors text-sm";

  if (cases.length === 0) {
    return (
      <div className="bg-white border border-sindicato-charcoal/10 p-10 text-center">
        <p className="text-sindicato-charcoal/50 text-sm font-[family-name:var(--font-barlow)] uppercase tracking-wider mb-4">
          No cases filed yet
        </p>
        <Link
          href="/file"
          className="bg-sindicato-bordeaux text-sindicato-cream px-6 py-2.5 text-sm font-bold uppercase tracking-wider hover:bg-sindicato-bordeaux-dark transition-colors font-[family-name:var(--font-barlow)] inline-block"
        >
  ...
  return (
    <div className="space-y-4">
      {cases.map((c) => {
        ...
        return (
          <div key={c.id} className="bg-white border border-sindicato-charcoal/10">
            {/* Case header */}
            <button
              type="button"
              onClick={() => setExpandedCaseId(isExpanded ? null : c.id)}
              className="w-full text-left p-5 flex items-start justify-between gap-4 hover:bg-sindicato-parchment/20 transition-colors"
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  <span className="text-sm font-bold text-sindicato-charcoal font-[family-name:var(--font-barlow)] uppercase tracking-wider">
                    {c.companyName}
                  </span>
                  {c.caseType && (
                ...
                <div className="flex items-center gap-3 flex-wrap">
                  <span className={`text-xs px-2 py-0.5 font-bold font-[family-name:var(--font-barlow)] uppercase tracking-wider ${RESOLUTION_COLORS[c.resolutionStatus] || RESOLUTION_COLORS.none}`}>
                    {RESOLUTION_LABELS[c.resolutionStatus] || c.resolutionStatus}
                  </span>
                  <span className="text-xs text-sindicato-charcoal/40">
                    Filed {format(new Date(c.createdAt), "d MMM yyyy")}
                  </span>
                  {parseFloat(c.amountOwed) > 0 && (
                    <span className="text-xs text-sindicato-charcoal/60">
              ...
              <svg
                className={`w-4 h-4 text-sindicato-charcoal/30 flex-shrink-0 mt-1 transition-transform ${isExpanded ? "rotate-180" : ""}`}
                fill="none"
                viewBox="0 0 16 16"
              >
            ...
            {isExpanded && (
              <div className="border-t border-sindicato-charcoal/10 p-5">
                    ...
                    {events.map((ev) => (
                      <div key={ev.id} className="flex items-start gap-3 text-sm">
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-sm text-sindicato-charcoal/70">{ev.description}</p>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {/* Add event form */}
                {isAddingEvent ? (
                  <div className="border border-sindicato-bordeaux/20 bg-sindicato-parchment/30 p-4 space-y-3">
                    <div>
                      <label className={labelClass}>Event type</label>
                      <select
                        value={draft.eventType}
                        onChange={(e) => setDrafts((d) => ({ ...d, [c.id]: { ...(d[c.id] || emptyDraft()), eventType: e.target.value } }))}
                    ...
                    <div>
                      ...
                      <input
                        ...
                        onChange={(e) => setDrafts((d) => ({ ...d, [c.id]: { ...(d[c.id] || emptyDraft()), eventDate: e.target.value } }))}
                        className={inputClass}
                      />
                    </div>
                    <div>
                      ...
                      <textarea
                        ...
                        onChange={(e) => setDrafts((d) => ({ ...d, [c.id]: { ...(d[c.id] || emptyDraft()), description: e.target.value } }))}
                    ...
                    <div className="flex gap-3">
                      <button
                        ...
                        className="bg-sindicato-bordeaux text-sindicato-cream px-4 py-1.5 text-xs font-bold uppercase tracking-wider font-[family-name:var(--font-barlow)] hover:bg-sindicato-bordeaux-dark transition-colors disabled:opacity-50"
                      >
                        {saving ? "Saving..." : "Save"}
                      </button>
                      <button
                        type="button"
                        onClick={() => { setShowAddEventFor(null); setSaveError(""); }}
                        className="text-sindicato-charcoal/50 text-xs uppercase tracking-wider font-[family-name:var(--font-barlow)] hover:text-sindicato-charcoal transition-colors"
                      >
                        Cancel
                      </button>
                    </div>
                  ...
                  <button
                    type="button"
                    onClick={() => { setShowAddEventFor(c.id); setSaveError(""); }}

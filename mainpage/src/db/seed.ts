...
const remoteCompanies = [
  { slug: "alignerr", name: "Alignerr", websiteUrl: "https://alignerr.com" },
  { slug: "labelbox", name: "Labelbox", websiteUrl: "https://labelbox.com" },
  { slug: "appen", name: "Appen", websiteUrl: "https://appen.com" },
  { slug: "scale-ai", name: "Scale AI", websiteUrl: "https://scale.com" },
  { slug: "toloka", name: "Toloka", websiteUrl: "https://toloka.ai" },
];

const gigCompanies = [
  { slug: "uber", name: "Uber", websiteUrl: "https://uber.com" },
  { slug: "glovo", name: "Glovo", websiteUrl: "https://glovoapp.com" },
  { slug: "doordash", name: "DoorDash", websiteUrl: "https://doordash.com" },
  { slug: "just-eat", name: "Just Eat", websiteUrl: "https://just-eat.com" },
  { slug: "gigable", name: "Gigable", websiteUrl: "https://gigable.com" },
];

const remoteCases: Array<{
  displayName: string;
  country: string;
  projects: string;
  dateRange: string;
  ...
  story: string;
  email: string;
  claimTypes: { unpaidWages?: boolean; unfairPractices?: boolean; retaliation?: boolean; other?: boolean };
...
}> = [
  {
    ...
    country: "Brazil",
    projects: "Alignerr projects (AI training data, multiple pipelines)",
    dateRange: "Apr 9 2026 - May 18 2026",
    amountOwed: "2432.00",
    currency: "USD",
    contactAttempts: 7,
    email: "victor@example.com",
    claimTypes: { unpaidWages: true, unfairPractices: true },
  ...
  },
  {
    displayName: "Ana P",
    country: "Portugal",
    projects: "Alignerr AI training validation project",
  ...
  },
  {
    displayName: "Ahmed K",
    country: "Egypt",
    projects: "Speech recognition data collection and validation",
    dateRange: "Nov 2024 - Jul 2025",
    amountOwed: "9200.00",
    currency: "USD",
    contactAttempts: 12,
    email: "ahmedk@example.com",
    claimTypes: { unpaidWages: true, unfairPractices: true, other: true },
    otherDescription: "Platform locked my account with funds inside",
  ...
  },
  {
    displayName: "Sarah L",
    country: "Philippines",
    projects: "3D point cloud annotation for LiDAR training (5 projects)",
    dateRange: "Feb 2025 - Sep 2025",
    amountOwed: "15700.00",
    currency: "USD",
    contactAttempts: 6,
    email: "sarahl@example.com",
    claimTypes: { unpaidWages: true, unfairPractices: true, retaliation: true },
  ...
  },
  {
    ...
    country: "Venezuela",
    projects: "Search relevance evaluation and data categorization",
...
const gigCases: Array<{
...
}> = [
  {
    ...
    country: "Spain",
    projects: "Food delivery (Uber Eats) in Barcelona, 8 months",
  ...
  },
  {
    displayName: "Ana S",
    country: "Portugal",
    projects: "Grocery delivery (Glovo) in Lisbon, 6 months",
    dateRange: "Mar 2025 - Aug 2025",
    amountOwed: "2800.00",
    currency: "EUR",
    contactAttempts: 5,
    email: "anas@example.com",
    claimTypes: { unpaidWages: true, retaliation: true },
  ...
  },
  {
    ...
    projects: "Food delivery (DoorDash) in London, 5 months",
  ...
  },
  {
    displayName: "Elena V",
    country: "Netherlands",
    projects: "Food delivery (Just Eat Takeaway) in Amsterdam, 4 months",
    dateRange: "May 2025 - Aug 2025",
    amountOwed: "1900.00",
    currency: "EUR",
    contactAttempts: 4,
    email: "elenav@example.com",
    claimTypes: { unpaidWages: true, other: true },
    otherDescription: "Misclassification as independent contractor, illegal deductions",
  ...
  },
  {
    displayName: "David O",
    country: "Nigeria",
    projects: "Food and grocery delivery across multiple apps, 6 months",
...
async function seed() {
  ...
  const workerEmails = [...new Set(allCaseData.map((c) => c.email))];
  ...
  const insertedWorkers = await db
    ...
    .values(
      workerEmails.map((email) => ({
        email,
        displayName: allCaseData.find((c) => c.email === email)!.displayName,
        emailVerified: true,
      }))
    )
    .returning({ id: workers.id, email: workers.email });

  const workerMap = new Map(insertedWorkers.map((w) => [w.email, w.id]));

  for (const caseData of allCaseData) {
    const companyId = companyMap.get(caseData.companySlug);
    const workerId = workerMap.get(caseData.email);
    if (!companyId || !workerId) {
      console.warn(`Skipping case for ${caseData.companySlug}: missing company or worker`);
      continue;
    }
    ...
    await db.insert(cases).values({
      ...
      claimTypes: caseData.claimTypes,
      otherDescription: caseData.otherDescription ?? null,
      attestation: true,
      consentLegal: true,
